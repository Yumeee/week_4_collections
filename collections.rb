...
room201 = collections[:data][:rooms][0][:capacity]
puts room201
id = nil
collections[:data][:rooms] do |room|
  if room[:room_number] == "201"
    id = room[:id]
  end
end

collections[:data][:events] do |event|
  if event[:room_id] == id && event[:attendees] <= room201
    puts "Ok!"
  end
end
